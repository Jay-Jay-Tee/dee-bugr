...
`;
  ...
  async attach(host, port, adapterType = "python") {
  ...
  async disassemble(memoryReference, count = 50) {
    return this.request("disassemble", {
      ...
      offset: 0,
      instructionOffset: 0,
      // FIX: was -10, now 0 — start from the actual instruction pointer
...
async function launchPythonAdapter(scriptPath) {
  ...
  return new Promise((resolve, reject) => {
    ...
    (_b = child.stderr) == null ? void 0 : _b.on("data", (data) => {
      ...
      if (!resolved && (msg.includes("Waiting for client to connect") || msg.includes("Listening on") || msg.includes("waiting for client"))) {
        resolved = true;
...
async function launchJavaAdapter() {
  ...
  await new Promise((resolve) => {
    ...
    (_b2 = child.stderr) == null ? void 0 : _b2.on("data", (d) => {
      if (d.toString().toLowerCase().includes("listening")) done();
    });
    child.on("exit", done);
    setTimeout(done, 2e3);
...
class SessionManager {
  constructor() {
    ...
    __publicField(this, "lastStepCommand", "other");
    __publicField(this, "lastLaunchArgs", null);
  ...
  async launch(language, scriptPath, breakpointLines = []) {
    ...
    if (language === "python") {
      const initializedPromise = new Promise((resolve) => {
        ...
        this.client.once("event:initialized", () => {
    ...
    try {
      ...
      const frames = rawFrames.map((f) => {
        ...
        return {
          ...
          instructionPointerReference: typeof f["instructionPointerReference"] === "string" ? str(f["instructionPointerReference"]) : void 0
        };
      });
      ...
      try {
        this.state.sourceLines = require$$0.readFileSync(this.state.currentFile, "utf8").split("\n");
      } catch {
      }
      if ((this.language === "c" || this.language === "cpp" || this.language === "java") && frames.length > 0) {
        const instrPtr = frames[0].instructionPointerReference;
        if (instrPtr) {
          try {
  ...
  async switchThread(newThreadId) {
...
function formatVariables(variables) {
  ...
  let output = "";
  for (const v of variables) {
    const truncatedValue = v.value.length > MAX_VALUE_LENGTH ? v.value.substring(0, MAX_VALUE_LENGTH) + "..." : v.value;
...
function getSourceSnippet(sourceLines, currentLine, contextLines) {
...
function getExecutionHistory() {
  ...
  const lines = history.slice(0, 10).map((entry, idx) => {
    const changedVars = Object.entries(entry.variables).filter(([_, v]) => v.changed).map(([name, v]) => `${name}=${v.value}`).join(", ");
...
function ensureApiKey() {
  if (!GROQ_API_KEY) {
    throw new Error(
      "Groq API key not found. Set DEE_BUGR_GROQ_KEY environment variable. Get a free key from https://console.groq.com"
    );
  }
}
async function callGroq(systemPrompt, userPrompt) {
  ...
  try {
    ...
    return ((_c = (_b = (_a = data.choices) == null ? void 0 : _a[0]) == null ? void 0 : _b.message) == null ? void 0 : _c.content) || "No response from AI";
  } catch (err) {
...
async function explainBug() {
  ...
  const systemPrompt = "You are an expert debugger. Analyze the given execution context and explain what bug or issue is likely present. Be concise but thorough. Suggest the root cause and how to fix it.";
  const userPrompt = `
Debug Context:
...
async function suggestFix() {
  const context = getDebugContext();
  const systemPrompt = 'You are an expert code reviewer. Given the execution context, suggest a specific code fix. Return ONLY a valid JSON object with exactly these keys: { "originalCode": "the buggy lines", "fixedCode": "the corrected lines", "explanation": "one sentence why" }. No markdown. No backticks. No extra text. Only the JSON object.';
  const userPrompt = `
Debug Context:
Language: ${context.language}
File: ${context.currentFile}, Line: ${context.currentLine}
...
async function explainVariable(varName) {
  const context = getDebugContext();
  const systemPrompt = "You are a debugging assistant. Explain what the named variable does and why its current value might be problematic or correct.";
  const userPrompt = `
...
async function generateWatch() {
  const context = getDebugContext();
  const systemPrompt = 'You are a debugging assistant. Given the execution context, suggest 3-5 variables or expressions the developer should watch to debug this issue. Return ONLY a JSON array of strings, nothing else.\nExample: ["length", "ptr", "i < array.length", "count > 0"]';
  const userPrompt = `
Language: ${context.language}
...
async function suggestBreakpoints(sourceCode, language) {
  const systemPrompt = `You are a debugging expert for ${language}. Analyze the source code and suggest 3-5 breakpoint locations that would be most useful for understanding the program flow.
...
async function sessionNarrative() {
  const history = getExecutionHistory();
  const context = getDebugContext();
  const systemPrompt = "You are a software journalist. Write a brief, engaging summary of the debug session's execution flow and findings.";
  const userPrompt = `
...
ipcMain.handle(IPC.READ_FILE, async (_, path2) => {
