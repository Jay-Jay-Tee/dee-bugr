...
// Day 7: AI panel + structured diff + anomaly banner + object graph tab
...
function highlightAsm(instruction: string): React.ReactNode {
  ...
  const mnemonic = parts[0]; const rest = parts.slice(1).join(' ')
  const color = /^j/i.test(mnemonic) ? 'text-[#c586c0]' : /^call/i.test(mnemonic) ? 'text-[#dcdcaa]' :
                /^ret/i.test(mnemonic) ? 'text-[#f48771]' : /^mov/i.test(mnemonic) ? 'text-[#9cdcfe]' : 'text-[#569cd6]'
  return <><span className={`${color} font-medium`}>{mnemonic}</span>{rest && <span className="text-[#cccccc]"> {rest}</span>}</>
}
...
function AssemblyPanel() {
  ...
  useEffect(() => { currentLineRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' }) }, [assemblyLines, currentLine])

  if (isBeginnerMode) return <div className="flex-1 flex items-center justify-center p-4 text-center text-[#555] text-xs">Assembly view is hidden in Beginner mode.<br />Switch to Expert mode.</div>
  if (language !== 'c' && language !== 'cpp' && language !== 'java') return <div className="flex-1 flex items-center justify-center text-[#555] text-xs p-4 text-center">Assembly available for C / C++ / Java.</div>
  if (status === 'idle' || status === 'terminated') return <div className="flex-1 flex items-center justify-center text-[#555] text-xs">Launch a session to see assembly</div>
  if (assemblyLines.length === 0) return <div className="flex-1 flex items-center justify-center text-[#555] text-xs">{status === 'paused' ? 'No disassembly for this frame' : 'Step to a breakpoint'}</div>
  ...
  return (
    <div className="flex-1 flex flex-col p-3 overflow-hidden">
      <div className="flex gap-2 mb-3 shrink-0">
        ...
        {explanation && <button onClick={() => navigator.clipboard.writeText(explanation)} className="px-2 py-1 text-xs rounded font-medium bg-[#3c3c3c] text-[#ccc] hover:bg-[#4a4a4a]">📋 Copy</button>}
      </div>
      <div className="flex-1 overflow-y-auto">
        ...
        {!explanation && !loading && !error && <div className="text-[#555] text-xs">Hit a breakpoint then click Explain Bug.</div>}
      </div>
    </div>
  )
}
...
function diffLines(original: string, fixed: string) {
  ...
  for (let i = 0; i < max; i++) {
    ...
    else { if (o !== undefined) result.push({ type: 'remove', text: o }); if (f !== undefined) result.push({ type: 'add', text: f }) }
  }
  return result
}
...
function AIFixPanel() {
  ...
  const fetchFix = async () => {
    ...
    try {
      ...
      if (result.success && result.fix) setFix(result.fix as FixResult)
      else setError(result.error || 'AI call failed')
    } catch (e: any) { setError(e.message || 'Unknown error') }
    finally { setLoading(false) }
  }
  ...
  return (
    <div className="flex-1 flex flex-col p-3 overflow-hidden">
      <div className="flex gap-2 mb-3 shrink-0">
        ...
        {fix && !accepted && (
          <>
            <button onClick={() => { navigator.clipboard.writeText(fix.fixedCode); setAccepted(true) }} className="px-2 py-1 text-xs rounded font-medium bg-green-800 text-white hover:bg-green-700">✓ Accept</button>
            <button onClick={() => { setFix(null); setError('') }} className="px-2 py-1 text-xs rounded font-medium bg-[#3c3c3c] text-[#ccc] hover:bg-[#4a4a4a]">✗ Reject</button>
          </>
        )}
      </div>
      <div className="flex-1 overflow-y-auto">
              ...
              <div className="font-mono text-xs rounded overflow-hidden border border-[#3c3c3c] mb-3">
                ...
                {diffResult.map((line, i) => (
                  <div key={i} className={['px-3 py-0.5 whitespace-pre-wrap break-all',
                    line.type === 'remove' ? 'bg-red-950/50 text-red-300' : line.type === 'add' ? 'bg-green-950/50 text-green-300' : 'text-[#888]'].join(' ')}>
...
function ObjectGraphPanel() {
  ...
  if (status === 'idle' || status === 'terminated') return <div className="flex-1 flex items-center justify-center text-[#555] text-xs">Launch a session to visualize objects</div>
  if (objectVars.length === 0) return <div className="flex-1 flex items-center justify-center text-[#555] text-xs text-center px-4">No objects yet.<br /><span className="text-[#444] mt-1 block">Step to a frame with objects.</span></div>
      ...
      <div className="flex-1 overflow-y-auto space-y-1">
        {objectVars.map((v) => (
          <div key={v.name} className="flex items-center gap-2 p-2 rounded bg-[#2a2a2a] hover:bg-[#333] cursor-pointer text-xs">
            ...
            {v.memoryReference && !isBeginnerMode && <span className="text-[10px] text-[#444] ml-auto shrink-0 font-mono">{v.memoryReference}</span>}
          </div>
        ))}
      </div>
